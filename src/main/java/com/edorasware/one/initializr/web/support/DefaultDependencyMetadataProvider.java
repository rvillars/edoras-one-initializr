...
public class DefaultDependencyMetadataProvider implements DependencyMetadataProvider {
	...
	public DependencyMetadata get(InitializrMetadata metadata, Version edorasoneVersion) {
		Map<String, Dependency> dependencies = new LinkedHashMap<>();
		for (Dependency d : metadata.getDependencies().getAll()) {
			if (d.match(edorasoneVersion)) {
				dependencies.put(d.getId(), d.resolve(edorasoneVersion));
			}
		}
		...
		for (Dependency d : dependencies.values()) {
			if (d.getBom() != null) {
				boms.put(d.getBom(), metadata.getConfiguration().getEnv()
						.getBoms().get(d.getBom()).resolve(edorasoneVersion));
			}
		}
		...
		return new DependencyMetadata(edorasoneVersion, dependencies, repositories, boms);
	}
