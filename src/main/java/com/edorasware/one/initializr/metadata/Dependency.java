...
import com.edorasware.one.initializr.util.InvalidVersionException;
import com.edorasware.one.initializr.util.Version;
import com.edorasware.one.initializr.util.VersionParser;
import com.edorasware.one.initializr.util.VersionRange;
...
public class Dependency extends MetadataElement implements Describable {
	...
	/**
	 * Define this dependency as a standard edoras one starter with the specified name
	 * <p>
	 * If no name is specified, the root "edoras-one-starter" is assumed.
	 */
	public Dependency asEdorasoneStarter(String name) {
		groupId = "com.edorasware.one";
		artifactId = StringUtils.hasText(name) ? "edoras-one-starter-" + name
				: "edoras-one-starter";
		version = "${com.edorasware.one.starter.version}";
	...
	public void resolve() {
		if (getId() == null) {
			...
			if (st.countTokens() == 1) { // assume edoras-one-starter
				asEdorasoneStarter(getId());
			}
	...
	/**
	 * Resolve this instance according to the specified edoras one {@link Version}.
	 * Return a {@link Dependency} instance that has its state resolved against the
	 * specified version.
	 */
	public Dependency resolve(Version edorasoneVersion) {
		for (Mapping mapping : mappings) {
			if (mapping.range.match(edorasoneVersion)) {
	...
	/**
	 * Specify if this dependency is available for the specified edoras one version.
	 */
	 ...
	 * to the edoras one version. If no mapping matches, default attributes are used.
	 */
